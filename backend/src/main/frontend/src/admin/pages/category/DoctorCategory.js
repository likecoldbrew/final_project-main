...
import { FileText, FolderPlus, Loader, Send, Settings, User } from "react-feather";
...
const icons = {
  ...
  "setting": <Settings size={20} /> // 마이페이지
  // 필요한 만큼 아이콘 추가
};
...
export default function DoctorCategory() {
  ...
  const saveOrder = async () => {
    ...
    try {
      const response = await fetch("/api/category/doctor/update/order", {
        method: "PUT",
  ...
  const saveName = async () => {
    ...
    try {
      const response = await fetch(`/api/category/doctor/update/name`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(newCategories)
      });
                  ...
                  <Draggable key={category.key} draggableId={String(category.key)} index={index}>
                    {(provided) => (
                      <li ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                          className={`bg-rose-50 p-2 rounded-md shadow-md items-center`}>
                        <div className="flex items-center justify-between">
                          <div
                            className={`p-2 flex items-center space-x-3 rounded-md ${isEditing ? "bg-green-100" : "bg-rose-50"}`}>
                            {category.icon}
                            <input
                              type="text"
                              value={category.name}
                              ...
                              readOnly={!isEditing}
                              className={`py-1 focus:outline-none ${isEditing ? "bg-green-100 border-gray-300 focus:border-gray-500 border-b" : "bg-rose-50"}`}
                            />
                          </div>
                        </div>
                                ...
                                <Draggable key={`sub-${category.key}-${subIndex}`}
                                  ...
                                  {(provided) => (
                                    <li
                                      ...
                                      className={`px-3 py-2 shadow rounded-md items-center ${isEditing ? "bg-green-50 border-gray-300 focus:border-gray-500" : "bg-rose-100"}`}>
                                      <input
                                        type="text"
                                        value={sub.name}
                                        onChange={(e) => handleSubcategoryNameChange(index, subIndex, e.target.value)}
                                        readOnly={!isEditing}
                                        className={`py-1 focus:outline-none ${isEditing ? "bg-green-50 border-gray-300 focus:border-gray-500 border-b" : "bg-rose-100"}`}
                                      />
                                    </li>
                                  )}
                                </Draggable>
