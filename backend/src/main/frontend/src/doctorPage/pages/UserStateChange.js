...
export const UserStateChange = ({
  ...
  useEffect(() => {
    if (initialPrescriptionInfo) {
      setPrescriptionInfo({
        medicalRecordId: initialPrescriptionInfo.medicalRecordId || "",
        userNo: initialPatientInfo.userNo || "",
        userName: initialPatientInfo.userName || "",
        doctorNo: initialPatientInfo.doctorNo || "",
  ...
  const handleRecordSubmit = async () => {
    ...
    try {
      ...
      // //응답에서 medical_recordId를 추출
      // const idResponse = await fetch("/api/medical_record/getLastInsertId");
      // if (!idResponse.ok) {
      //   const errorText = await idResponse.text();
      //   throw new Error(`Failed to retrieve last insert ID: ${errorText}`);
      // }
      ...
      // 두 번째 API 호출: 처방 추가
      // const prescriptionResponse = await fetch("/api/prescription/insertAddPrescription", {
      //   method: "POST",
      //   headers: {
      //     "Content-Type": "application/json"
      //   },
      //   body: JSON.stringify({
      //     medicalRecordId: recordId, // 첫 번째 API 호출에서 받은 ID를 사용
      //     userNo: patientInfo.userNo, // 환자 사용자 번호 추가
      //     doctorNo: patientInfo.doctorNo, // 의사 번호 추가
      //     medicationName: prescriptionInfo.medicationName,
      //     dosage: prescriptionInfo.dosage,
