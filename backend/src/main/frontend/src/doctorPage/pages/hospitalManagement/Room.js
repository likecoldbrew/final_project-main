...
export default function Room({ isOpen, closeModal, currentItem }) {
  ...
  let inputIndex = 1; // 외부 변수로 순차 인덱스를 관리

  // 병실 정보 가져오기
  const handleGetRoomId = async (id, value) => {
    console.log(value);
    if (value) {
      alert("현재 사용 중인 병실입니다. 다른 방을 선택하세요");
      return;
    }

    const fetchRoomInfo = async () => {
      try {
        const response = await fetch(`/api/hospitalization/room/${id}`);
  ...
  const renderRoom = (roomNum, beds) => (
    <div key={roomNum} className="border-2 border-gray-800 p-2 aspect-square relative z-30">
        ...
        {[...Array(beds)].map((_, bedIndex) => {
          ...
          return (
            <div key={bedIndex} className="relative">
              <div
                className="absolute top-1 left-1 w-5 h-5 border border-gray-800 flex items-center justify-center text-xs bg-white">
                {bedIndex + 1}
              </div>
              <div className="h-full flex items-center justify-center">
                <div className="w-full h-3/4 mt-2 border border-gray-800 rounded-lg bg-white">
                  <input
                    ...
                    readOnly={true}
                    onClick={() => handleGetRoomId(currentIndex, room ? room.userName : "")}
                  />
                </div>
              </div>
            </div>
  ...
  return (
    <div className="fixed left-48 inset-0 flex items-center justify-center z-50">
